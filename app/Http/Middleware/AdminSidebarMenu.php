    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        if (auth()->user()->can('user.view')) {
                            $sub->url(
                                action('App\Http\Controllers\ManageUserController@index'),
                                __('user.users'),
                        ...
                        if (auth()->user()->can('user.create')) {
                            $sub->url(
                                action('App\Http\Controllers\SalesCommissionAgentController@index'),
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        if (auth()->user()->can('supplier.view') || auth()->user()->can('supplier.view_own')) {
                            $sub->url(
                                action('App\Http\Controllers\ContactController@index', ['type' => 'supplier']),
                                __('report.supplier'),
                        ...
                        if (auth()->user()->can('customer.view') || auth()->user()->can('customer.view_own')) {
                            $sub->url(
                                action('App\Http\Controllers\ContactController@index', ['type' => 'customer']),
                                __('report.customer'),
                                ['icon' => 'fa fas fa-star', 'active' => request()->input('type') == 'customer']
                            );
                            $sub->url(
                                action('App\Http\Controllers\CustomerGroupController@index'),
                        ...
                        if (auth()->user()->can('supplier.create') || auth()->user()->can('customer.create')) {
                            $sub->url(
                                action('App\Http\Controllers\ContactController@getImportContacts'),
                        ...
                        if(!empty(env('GOOGLE_MAP_API_KEY'))) {
                            $sub->url(
                                action('App\Http\Controllers\ContactController@contactMap'),
                                __('lang_v1.map'),
            ...
            if (auth()->user()->can('product.view') || auth()->user()->can('product.create') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('product.view')) {
                            $sub->url(
                                action('App\Http\Controllers\LabelsController@show'),
                        ...
                        if (auth()->user()->can('product.create')) {
                            $sub->url(
                                action('App\Http\Controllers\VariationTemplateController@index'),
                                __('product.variations'),
                                ['icon' => 'fa fas fa-circle', 'active' => request()->segment(1) == 'variation-templates']
                            );
                            $sub->url(
                                action('App\Http\Controllers\ImportProductsController@index'),
                        ...
                        if (auth()->user()->can('product.opening_stock')) {
                            $sub->url(
                                action('App\Http\Controllers\ImportOpeningStockController@index'),
                        ...
                        if (auth()->user()->can('product.create')) {
                            $sub->url(
                                action('App\Http\Controllers\SellingPriceGroupController@index'),
                        ...
                        if (auth()->user()->can('unit.view') || auth()->user()->can('unit.create')) {
                            $sub->url(
                                action('App\Http\Controllers\UnitController@index'),
                                __('unit.units'),
                        ...
                        if (auth()->user()->can('category.view') || auth()->user()->can('category.create')) {
                            $sub->url(
                                action('App\Http\Controllers\TaxonomyController@index') . '?type=product',
                        ...
                        $sub->url(
                            action('App\Http\Controllers\WarrantyController@index'),
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($common_settings) {
                        ...
                        if (auth()->user()->can('purchase.view') || auth()->user()->can('view_own_purchase')) {
                            $sub->url(
                                action('App\Http\Controllers\PurchaseController@index'),
                        ...
                        if (auth()->user()->can('purchase.create')) {
                            $sub->url(
                                action('App\Http\Controllers\PurchaseController@create'),
                        ...
                        if (auth()->user()->can('purchase.update')) {
                            $sub->url(
                                action('App\Http\Controllers\PurchaseReturnController@index'),
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                        ...
                        if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
                            $sub->url(
                                action('App\Http\Controllers\SellController@create'),
                                __('sale.add_sale'),
                        ...
                        if (auth()->user()->can('sell.create')) {
                            if (in_array('pos_sale', $enabled_modules)) {
                                if (auth()->user()->can('sell.view')) {
                                    $sub->url(
                                        action('App\Http\Controllers\SellPosController@index'),
                                        __('sale.list_pos'),
                                ...
                                $sub->url(
                                    action('App\Http\Controllers\SellPosController@create'),
                                    __('sale.pos_sale'),
                        ...
                        if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
                            $sub->url(
                                action('App\Http\Controllers\SellController@create', ['status' => 'draft']),
                        ...
                        if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
                            $sub->url(
                                action('App\Http\Controllers\SellController@create', ['status' => 'quotation']),
                        ...
                        if (auth()->user()->can('access_sell_return') || auth()->user()->can('access_own_sell_return')) {
                            $sub->url(
                                action('App\Http\Controllers\SellReturnController@index'),
                        ...
                        if ($is_admin || auth()->user()->hasAnyPermission(['access_shipping', 'access_own_shipping', 'access_commission_agent_shipping']) ) {
                            $sub->url(
                                action('App\Http\Controllers\SellController@shipments'),
                        ...
                        if (auth()->user()->can('discount.access')) {
                            $sub->url(
                                action('App\Http\Controllers\DiscountController@index'),
                        ...
                        if (in_array('subscription', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
                            $sub->url(
                                action('App\Http\Controllers\SellPosController@listSubscriptions'),
                        ...
                        if (auth()->user()->can('sell.create')) {
                            $sub->url(
                                action('App\Http\Controllers\ImportSalesController@index'),
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        if (auth()->user()->can('purchase.view')) {
                            $sub->url(
                                action('App\Http\Controllers\StockTransferController@index'),
                        ...
                        if (auth()->user()->can('purchase.create')) {
                            $sub->url(
                                action('App\Http\Controllers\StockTransferController@create'),
            ...
            if (in_array('stock_adjustment', $enabled_modules) && (auth()->user()->can('purchase.view') || auth()->user()->can('purchase.create'))) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        if (auth()->user()->can('purchase.view')) {
                            $sub->url(
                                action('App\Http\Controllers\StockAdjustmentController@index'),
                        ...
                        if (auth()->user()->can('purchase.create')) {
                            $sub->url(
                                action('App\Http\Controllers\StockAdjustmentController@create'),
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        if (auth()->user()->can('profit_loss_report.view')) {
                            $sub->url(
                                action('App\Http\Controllers\ReportController@getProfitLoss'),
                        ...
                        if (config('constants.show_report_606') == true) {
                            $sub->url(
                                action('App\Http\Controllers\ReportController@purchaseReport'),
                        ...
                        if (config('constants.show_report_607') == true) {
                            $sub->url(
                                action('App\Http\Controllers\ReportController@saleReport'),
                        ...
                        if ((in_array('purchases', $enabled_modules) || in_array('add_sale', $enabled_modules) || in_array('pos_sale', $enabled_modules)) && auth()->user()->can('purchase_n_sell_report.view')) {
                            $sub->url(
                                action('App\Http\Controllers\ReportController@getPurchaseSell'),
                        ...
                        if (auth()->user()->can('tax_report.view')) {
                            $sub->url(
                                action('App\Http\Controllers\ReportController@getTaxReport'),
                        ...
                        if (auth()->user()->can('contacts_report.view')) {
                            $sub->url(
                                action('App\Http\Controllers\ReportController@getCustomerSuppliers'),
                                __('report.contacts'),
                                ['icon' => 'fa fas fa-address-book', 'active' => request()->segment(2) == 'customer-supplier']
                            );
                            $sub->url(
                                action('App\Http\Controllers\ReportController@getCustomerGroup'),
                        ...
                        if (auth()->user()->can('stock_report.view')) {
                            $sub->url(
                                action('App\Http\Controllers\ReportController@getStockReport'),
                                __('report.stock_report'),
                                ['icon' => 'fa fas fa-hourglass-half', 'active' => request()->segment(2) == 'stock-report']
                            );
                            if (session('business.enable_product_expiry') == 1) {
                                $sub->url(
                                    action('App\Http\Controllers\ReportController@getStockExpiryReport'),
                            ...
                            if (session('business.enable_lot_number') == 1) {
                                $sub->url(
                                    action('App\Http\Controllers\ReportController@getLotReport'),
                            ...
                            if (in_array('stock_adjustment', $enabled_modules)) {
                                $sub->url(
                                    action('App\Http\Controllers\ReportController@getStockAdjustmentReport'),
                        ...
                        if (auth()->user()->can('trending_product_report.view')) {
                            $sub->url(
                                action('App\Http\Controllers\ReportController@getTrendingProducts'),
                        ...
                        if (in_array('expenses', $enabled_modules) && auth()->user()->can('expense_report.view')) {
                            $sub->url(
                                action('App\Http\Controllers\ReportController@getExpenseReport'),
                        ...
                        if (auth()->user()->can('register_report.view')) {
                            $sub->url(
                                action('App\Http\Controllers\ReportController@getRegisterReport'),
                        ...
                        if (auth()->user()->can('sales_representative.view')) {
                            $sub->url(
                                action('App\Http\Controllers\ReportController@getSalesRepresentativeReport'),
                        ...
                        if (auth()->user()->can('purchase_n_sell_report.view') && in_array('tables', $enabled_modules)) {
                            $sub->url(
                                action('App\Http\Controllers\ReportController@getTableReport'),
                        ...
                        if (auth()->user()->can('sales_representative.view') && in_array('service_staff', $enabled_modules)) {
                            $sub->url(
                                action('App\Http\Controllers\ReportController@getServiceStaffReport'),
                        ...
                        if ($is_admin) {
                            $sub->url(
                                action('App\Http\Controllers\ReportController@activityLog'),
            ...
            if (auth()->user()->can('backup')) {
                $menu->url(action('App\Http\Controllers\BackUpController@index'), __('lang_v1.backup'), ['icon' => 'fa fas fa-hdd', 'active' => request()->segment(1) == 'backup'])->order(60);
            }

            //Modules menu
            if (auth()->user()->can('manage_modules')) {
                $menu->url(action('App\Http\Controllers\Install\ModulesController@index'), __('lang_v1.modules'), ['icon' => 'fa fas fa-plug', 'active' => request()->segment(1) == 'manage-modules'])->order(60);
            }

            //Booking menu
            if (in_array('booking', $enabled_modules) && (auth()->user()->can('crud_all_bookings') || auth()->user()->can('crud_own_bookings'))) {
                $menu->url(action('App\Http\Controllers\Restaurant\BookingController@index'), __('restaurant.bookings'), ['icon' => 'fas fa fa-calendar-check', 'active' => request()->segment(1) == 'bookings'])->order(65);
            }

            //Kitchen menu
            if (in_array('kitchen', $enabled_modules)) {
                $menu->url(action('App\Http\Controllers\Restaurant\KitchenController@index'), __('restaurant.kitchen'), ['icon' => 'fa fas fa-fire', 'active' => request()->segment(1) == 'modules' && request()->segment(2) == 'kitchen'])->order(70);
            }

            //Service Staff menu
            if (in_array('service_staff', $enabled_modules)) {
                $menu->url(action('App\Http\Controllers\Restaurant\OrderController@index'), __('restaurant.orders'), ['icon' => 'fa fas fa-list-alt', 'active' => request()->segment(1) == 'modules' && request()->segment(2) == 'orders'])->order(75);
            }

            //Notification template menu
            if (auth()->user()->can('send_notifications')) {
                $menu->url(action('App\Http\Controllers\NotificationTemplateController@index'), __('lang_v1.notification_templates'), ['icon' => 'fa fas fa-envelope', 'active' => request()->segment(1) == 'notification-templates'])->order(80);
            }
            ...
            if (auth()->user()->can('business_settings.access') ||
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules) {
                        if (auth()->user()->can('business_settings.access')) {
                            $sub->url(
                                action('App\Http\Controllers\BusinessController@getBusinessSettings'),
                                __('business.business_settings'),
                                ['icon' => 'fa fas fa-cogs', 'active' => request()->segment(1) == 'business', 'id' => "tour_step2"]
                            );
                            $sub->url(
                                action('App\Http\Controllers\BusinessLocationController@index'),
                        ...
                        if (auth()->user()->can('invoice_settings.access')) {
                            $sub->url(
                                action('App\Http\Controllers\InvoiceSchemeController@index'),
                        ...
                        if (auth()->user()->can('barcode_settings.access')) {
                            $sub->url(
                                action('App\Http\Controllers\BarcodeController@index'),
                        ...
                        if (auth()->user()->can('access_printers')) {
                            $sub->url(
                                action('App\Http\Controllers\PrinterController@index'),
                        ...
                        if (in_array('tables', $enabled_modules) && auth()->user()->can('access_tables')) {
                            $sub->url(
                                action('App\Http\Controllers\Restaurant\TableController@index'),
                        ...
                        if (in_array('modifiers', $enabled_modules) && (auth()->user()->can('product.view') || auth()->user()->can('product.create'))) {
                            $sub->url(
                                action('App\Http\Controllers\Restaurant\ModifierSetsController@index'),
                        ...
                        if (in_array('types_of_service', $enabled_modules) && auth()->user()->can('access_types_of_service')) {
                            $sub->url(
                                action('App\Http\Controllers\TypesOfServiceController@index'),

		...
		@foreach ($transactions as $transaction)
			<tr class="cursor-pointer" 
				...
				<td>
					...
					<a href="{{action('App\Http\Controllers\SellPosController@edit', [$transaction->id]).$subtype}}">
	    			...
	    			<a href="{{action('App\Http\Controllers\SellPosController@destroy', [$transaction->id])}}" class="delete-sale" style="padding-left: 20px; padding-right: 20px"><i class="fa fa-trash text-danger" title="{{__('lang_v1.click_to_delete')}}"></i></a>
	    			@endif

	    			<a href="{{action('App\Http\Controllers\SellPosController@printInvoice', [$transaction->id])}}" class="print-invoice-link">

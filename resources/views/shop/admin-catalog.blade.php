...
@forelse($channels as $channel)<tr><td>{{ $channel->name }}</td><td>{{ $channel->slug }}</td><td>{{ $channel->products->count() }}</td><td>{{ $channel->enabled ? 'Enabled' : 'Disabled' }}</td><td><a class="btn btn-primary btn-sm" href="{{ route('shop.admin.catalog.products', $channel) }}">View products</a>@if($canManage) <a class="btn btn-default btn-sm" href="{{ route('shop.admin.catalog.channels.edit', $channel) }}">Edit channel</a>@endif</td></tr>@empty<tr><td colspan="5">No shop channel configured.</td></tr>@endforelse
</tbody></table></div></div></div>@if($canManage)<div class="col-md-5"><div class="box box-info"><div class="box-header"><h3 class="box-title">Create channel</h3></div><form method="post" action="{{ route('shop.admin.catalog.channels.store') }}">@csrf<div class="box-body">
...
</div><div class="box-footer"><button class="btn btn-primary">Create channel</button></div></form></div></div>@endif</div></section>
@endsection

...
@forelse($channels as $channel)<tr><td>{{ $channel->name }}</td><td>{{ $channel->slug }}</td><td>{{ $channel->products->count() }}</td><td>{{ $channel->enabled ? 'Enabled' : 'Disabled' }}</td><td><a class="btn btn-primary btn-sm" href="{{ route('shop.admin.catalog.products', $channel) }}">Manage products</a> <a class="btn btn-default btn-sm" href="{{ route('shop.admin.catalog.channels.edit', $channel) }}">Edit channel</a></td></tr>@empty<tr><td colspan="5">No shop channel configured.</td></tr>@endforelse

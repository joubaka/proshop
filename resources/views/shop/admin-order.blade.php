...
@if($order->payment_status === 'paid' && $order->fulfilment_status === 'not_ready')<form method="post" action="{{ route('shop.admin.orders.ready', $order->uuid) }}">@csrf<button class="btn btn-success btn-block">Mark ready for collection</button></form>@endif
@if($order->payment_status === 'paid' && $order->fulfilment_status === 'ready_for_collection')<form method="post" action="{{ route('shop.admin.orders.collected', $order->uuid) }}">@csrf<button class="btn btn-success btn-block">Mark collected</button></form>@endif
@if($order->payment_status === 'pending')<hr><form method="post" action="{{ route('shop.admin.orders.cancel', $order->uuid) }}">@csrf<label>Cancellation reason<input class="form-control" name="reason" minlength="5" maxlength="500" required></label><button class="btn btn-danger btn-block" type="submit">Cancel unpaid order</button></form>@endif
</div></div></div></div>
